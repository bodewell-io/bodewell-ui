// src/navigation/SideNav.tsx
import React, { useState, useEffect, useRef } from 'react';
import { Link, useLocation } from 'react-router-dom';

// A custom hook for tracking which section is currently in the viewport.
const useScrollSpy = (ids: string[], options: IntersectionObserverInit) => {
  const [activeId, setActiveId] = useState<string | null>(null);
  const observer = useRef<IntersectionObserver | null>(null);

  useEffect(() => {
    // A type guard is used here to ensure we only work with actual HTML elements.
    const elements = ids
...
// Exporting NavItem for use in other parts of the app, like the Styleguide itself.
 ...
 * @category navigation
 */
export const SideNav: React.FC<SideNavProps> = ({ navItems, sectionIds, className }) => {
